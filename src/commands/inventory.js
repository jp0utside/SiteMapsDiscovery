  ...
  if (cfg.inventory.bfs_link_crawl !== false && opts.crawl !== false) {
    ...
    const processOne = async (url) => {
      ...
      try { res = await http.getText(url, { htmlOnly: true }); }
        ...
        markRetry.run(String(e.message).slice(0, 300), now(), new Date(Date.now() + backoff).toISOString(), max, max, url);
      ...
      if (isBlockStatus(res.status)) {
        ...
        markRetry.run(`HTTP ${res.status}`, now(), new Date(Date.now() + backoff).toISOString(), max, max, url);
        log.warn(`HTTP ${res.status} for ${url} — treated as a block/outage, will retry (${health.summary()})`);
        return;
      }
      ...
      const tx = db.transaction(() => {
        ...
        if (runTier1) {
          ...
          const newApps = store.store(url, 1, det.findings, null);
    ...
    while (!stopping) {
      const batch = claim.all(now(), concurrency * 4).map(r => r.url);
      if (!batch.length) {
        const waiting = db.prepare(`SELECT COUNT(*) c FROM urls WHERE links_done=0 AND status IN ('pending','done') AND retry_after > ?`).get(now()).c;
        if (!waiting) break;
        log.info(`${waiting} URLs waiting for their retry window; pausing 15s`); await new Promise(r => setTimeout(r, 15000)); continue;
      }
      let i = 0;
